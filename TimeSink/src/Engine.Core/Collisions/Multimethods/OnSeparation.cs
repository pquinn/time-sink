...
// multimethod.py --result=void --name=OnSeparation --arg=ICollideable:obj1 --arg=Fixture:f1 --arg=ICollideable:obj2 --arg=Fixture:f2 --namespace=TimeSink.Engine.Core.Collisions
using System;
...
{
    // Dynamically dispatches to the best OnSeparation overload.
    public static class OnSeparation
    {
        ...
        public static void Invoke(ICollideable obj1, Fixture f1, ICollideable obj2, Fixture f2)
        {
            if (ms_candidates == null)
                DoAutoRegister();

            var __actual = new Entry(obj1, f1, obj2, f2);

            var __index = -1;

            while ((__index = DoGetNextMethod(__actual, __index + 1)) != -1)
                ms_candidates[__index].Method(obj1, f1, obj2, f2);
        }
        ...
        // MissingMethodException or System.Reflection.AmbiguousMatchException.
        public static void NextMethod(ICollideable obj1, Fixture f1, ICollideable obj2, Fixture f2)
        {
            var __actual = new Entry(obj1, f1, obj2, f2);

            int __index = ms_nextIndex;
            if (__index == -1)
                throw new MissingMethodException(string.Format("NextMethod failed to find an overload for OnSeparation({0}).", __actual));

            ms_nextIndex = DoGetNextMethod(__actual, __index + 1);
            if (ms_nextIndex > 0)
                if (DoLeftPrecedesRight(ms_candidates[__index], ms_candidates[ms_nextIndex]) == 0)
                    throw new AmbiguousMatchException(string.Format("NextMethod found ambiguous overloads for OnSeparation({0}).", __actual));
        ...
        public static void Register(Type type)
        {
            ...
            foreach (MethodInfo info in type.GetMethods(MethodBinding))
            {
                if (info.IsDefined(typeof(OverloadAttribute), false))
                {
                    ...
                    {
                        entry = new Entry(
                            (ICollideable a1, Fixture a2, ICollideable a3, Fixture a4) => temp.Invoke(null, new object[] { a1, a2, a3, a4 }),
                            parms[0].ParameterType, parms[1].ParameterType, parms[2].ParameterType, parms[3].ParameterType);
                    }
                    else
                    {
                        entry = new Entry(
                            (ICollideable a1, Fixture a2, ICollideable a3, Fixture a4) => temp.Invoke(a1, new object[] { a2, a3, a4 }),
        ...
        private delegate void OverloadType(ICollideable obj1, Fixture f1, ICollideable obj2, Fixture f2);

        private struct Entry : IEquatable<Entry>
        {
            public Entry(ICollideable obj1, Fixture f1, ICollideable obj2, Fixture f2)
                : this()
            {
                Method = null;
                Type1 = (object)obj1 != null ? obj1.GetType() : null;
                Type2 = (object)f1 != null ? f1.GetType() : null;
                Type3 = (object)obj2 != null ? obj2.GetType() : null;
                Type4 = (object)f2 != null ? f2.GetType() : null;
            }

            public Entry(OverloadType overload, Type type1, Type type2, Type type3, Type type4)
                : this()
            {
                Method = overload;
                Type1 = type1;
        ...
        public static void DoCheckMethod(MethodInfo info)
        {
            ...
            if (info.IsStatic)
            {
                ...
                if (!typeof(Fixture).IsAssignableFrom(parms[1].ParameterType))
                    throw new InvalidOperationException(string.Format("{0}.{1} argument {2} is not compatible with type Fixture.", info.DeclaringType.Name, info.Name, parms[1].Name));
                if (!typeof(ICollideable).IsAssignableFrom(parms[2].ParameterType))
                    throw new InvalidOperationException(string.Format("{0}.{1} argument {2} is not compatible with type ICollideable.", info.DeclaringType.Name, info.Name, parms[2].Name));
                if (!typeof(Fixture).IsAssignableFrom(parms[3].ParameterType))
                    throw new InvalidOperationException(string.Format("{0}.{1} argument {2} is not compatible with type Fixture.", info.DeclaringType.Name, info.Name, parms[3].Name));
            }
            else
            {
                if (parms.Length != 3)
                ...
                if (!typeof(Fixture).IsAssignableFrom(parms[0].ParameterType))
                    throw new InvalidOperationException(string.Format("{0}.{1} argument {2} is not compatible with type Fixture.", info.DeclaringType.Name, info.Name, parms[0].Name));
                ...
                if (!typeof(Fixture).IsAssignableFrom(parms[2].ParameterType))
                    throw new InvalidOperationException(string.Format("{0}.{1} argument {2} is not compatible with type Fixture.", info.DeclaringType.Name, info.Name, parms[2].Name));
            }
        }
        #endregion
