...
// C:\Users\mvmud_001\Downloads\multimethod.py --name=OnSeparation --arg=ICollideable:obj1 --arg=ICollideable:obj2 --namespace=TimeSink.Engine.Core.Collisions --result=void
using System;
...
namespace TimeSink.Engine.Core.Collisions
{
	...
	public static class OnSeparation
	{
		...
		public static void Invoke(ICollideable obj1, ICollideable obj2)
		{
			if (ms_candidates == null)
				DoAutoRegister();
			
			var __actual = new Entry(obj1, obj2);

            var __index = -1;

            while ((__index = DoGetNextMethod(__actual, __index + 1)) != -1)
                ms_candidates[__index].Method(obj1, obj2);
		/*	ms_nextIndex = DoGetNextMethod(__actual, __index + 1);
			if (ms_nextIndex > 0)
				if (DoLeftPrecedesRight(ms_candidates[__index], ms_candidates[ms_nextIndex]) == 0)
					throw new AmbiguousMatchException(string.Format("Invoke found ambiguous overloads for OnSeparation({0}).", __actual));
		...
		// MissingMethodException or System.Reflection.AmbiguousMatchException.
		public static void NextMethod(ICollideable obj1, ICollideable obj2)
		{
			var __actual = new Entry(obj1, obj2);
		...
		public static void Register(Type type)
		{
			...
			foreach (MethodInfo info in type.GetMethods(MethodBinding))
			{
				if (info.IsDefined(typeof(OverloadAttribute), false))
				{
					...
					{
						entry = new Entry(
							(ICollideable a1, ICollideable a2) => temp.Invoke(null, new object[]{a1, a2}),
							parms[0].ParameterType, parms[1].ParameterType);
					}
					else
					{
						entry = new Entry(
							(ICollideable a1, ICollideable a2) => temp.Invoke(a1, new object[]{a2}),
		...
		private delegate void OverloadType(ICollideable obj1, ICollideable obj2);
		
		private struct Entry : IEquatable<Entry>
		{
			public Entry(ICollideable obj1, ICollideable obj2) : this()
			{
				Method = null;
