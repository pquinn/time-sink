...
// multimethod.py --result=CollisionInfo --name=Collided --arg=ICollisionGeometry:shape1 --arg=ICollisionGeometry:shape2 --namespace=TimeSink.Engine.Core.Collisions
using System;
...
{
	// Dynamically dispatches to the best Collided overload.
	internal static class Collided
	{
		...
		public static CollisionInfo Invoke(ICollisionGeometry shape1, ICollisionGeometry shape2)
		{
			if (ms_candidates == null)
				DoAutoRegister();
			
			var __actual = new Entry(shape1, shape2);
			int __index = DoGetNextMethod(__actual, 0);
			if (__index == -1)
				throw new MissingMethodException(string.Format("Invoke failed to find an overload for Collided({0}).", __actual));
			
			ms_nextIndex = DoGetNextMethod(__actual, __index + 1);
			if (ms_nextIndex > 0)
				if (DoLeftPrecedesRight(ms_candidates[__index], ms_candidates[ms_nextIndex]) == 0)
					throw new AmbiguousMatchException(string.Format("Invoke found ambiguous overloads for Collided({0}).", __actual));
			
			return ms_candidates[__index].Method(shape1, shape2);
		}
		...
		// MissingMethodException or System.Reflection.AmbiguousMatchException.
		public static CollisionInfo NextMethod(ICollisionGeometry shape1, ICollisionGeometry shape2)
		{
			var __actual = new Entry(shape1, shape2);
			
			int __index = ms_nextIndex;
			if (__index == -1)
				throw new MissingMethodException(string.Format("NextMethod failed to find an overload for Collided({0}).", __actual));
			
			ms_nextIndex = DoGetNextMethod(__actual, __index + 1);
			if (ms_nextIndex > 0)
				if (DoLeftPrecedesRight(ms_candidates[__index], ms_candidates[ms_nextIndex]) == 0)
					throw new AmbiguousMatchException(string.Format("NextMethod found ambiguous overloads for Collided({0}).", __actual));
		...
		public static void Register(Type type)
		{
			...
			foreach (MethodInfo info in type.GetMethods(MethodBinding))
			{
				if (info.IsDefined(typeof(OverloadAttribute), false))
				{
					...
					{
						entry = new Entry(
							(ICollisionGeometry a1, ICollisionGeometry a2) => (CollisionInfo) temp.Invoke(null, new object[]{a1, a2}),
							parms[0].ParameterType, parms[1].ParameterType);
					}
					else
					{
						entry = new Entry(
							(ICollisionGeometry a1, ICollisionGeometry a2) => (CollisionInfo) temp.Invoke(a1, new object[]{a2}),
		...
		private delegate CollisionInfo OverloadType(ICollisionGeometry shape1, ICollisionGeometry shape2);
		
		private struct Entry : IEquatable<Entry>
		{
			public Entry(ICollisionGeometry shape1, ICollisionGeometry shape2) : this()
			{
				Method = null;
				Type1 = (object) shape1 != null ? shape1.GetType() : null;
				Type2 = (object) shape2 != null ? shape2.GetType() : null;
			}
		...
		public static void DoCheckMethod(MethodInfo info)
		{
			...
			if (!typeof(CollisionInfo).IsAssignableFrom(info.ReturnType))
				throw new InvalidOperationException(string.Format("{0}.{1} has return type {2} but should have CollisionInfo (or a derived class).", info.DeclaringType.Name, info.Name, info.ReturnType));
			...
			if (info.IsStatic)
			{
				...
				if (!typeof(ICollisionGeometry).IsAssignableFrom(parms[0].ParameterType))
					throw new InvalidOperationException(string.Format("{0}.{1} argument {2} is not compatible with type ICollisionGeometry.", info.DeclaringType.Name, info.Name, parms[0].Name));
				if (!typeof(ICollisionGeometry).IsAssignableFrom(parms[1].ParameterType))
					throw new InvalidOperationException(string.Format("{0}.{1} argument {2} is not compatible with type ICollisionGeometry.", info.DeclaringType.Name, info.Name, parms[1].Name));
			}
			else
			{
				if (parms.Length != 1)
				...
				if (!typeof(ICollisionGeometry).IsAssignableFrom(info.DeclaringType))
					throw new InvalidOperationException(string.Format("{0}.{1} this argument is not compatible with type ICollisionGeometry.", info.DeclaringType.Name, info.Name));
